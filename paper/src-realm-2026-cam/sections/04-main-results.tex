...
\paragraph{Data} We randomly generate 30 hidden messages per task, per message length ($\lvert\mathbf{h}\rvert$), from which we generate 30 stegotexts. For \qa, we vary $\lvert\mathbf{h}\rvert \in \{6,8,10\}$; for \lr and \sg, we vary $\lvert\mathbf{h}\rvert \in \{14,16,18\}$.\footnote{The largest message lengths for each task is chosen from a capacity sweep on an initial dry-run (3 secret message per scheme, 2 stegotexts per secret message, and 5 attack-decode trials per stegotext). \qa has ${\sim}$100\% perfect recovery rate at 10 bits, \sg at 18 bits, and \lr at 18 bits.}.
We report bitwise and perfect (full) message recovery accuracy over each set of 30 stegotexts.

\paragraph{Models} We use Qwen3.5 4B as the generator $G$ and GPT-4.1 to synthesize $\mathbf{s}$ and to carry out all attacks.\footnote{\autoref{app:implementation-details} has further model details.}
...
We find that for all three tasks, our scheme is relatively robust to the attacks. Notably, bitwise accuracy on \sg and \qa is nearly perfect up through the maximum message length for each task. Bitwise results for \lr are comparably strong under \rtt and the local variant of \llmp, with lesser but still strong results under the other two attacks.
...
By contrast, even with a generous repetition code\footnote{More than 100 repetitions, interleaved and decoded by majority voting, see \autoref{app:implementation-details}}, Discop shows at-chance bitwise recovery rates and a perfect recovery rate of zero, demonstrating that token-based generative steganography is not viable under these attacks.

% Lastly, we note that none of these results rely on any ECCs, which would likely improve perfect recovery rates across the board.
