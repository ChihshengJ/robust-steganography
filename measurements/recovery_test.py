...
def init_attacks(
    ...
    if attack_configs is None:
        ...
        return {
            ...
            "paraphrase": ParaphraseAttack(
                client=client, model="gpt-4o", temperature=0
            ),
    ...
    for cfg in attack_configs:
        ...
        if cfg.attack_type == "n-gram":
            ...
            attacks[cfg.label] = SynonymAttack(method="wordnet", local_mode=local_mode)
            ...
            attacks[cfg.label] = TranslationAttack(
                client=client, model="gpt-4o", temperature=0, local_mode=local_mode
            )
        else:
...
def compute_recovery_accuracy(original: list[int], recovered: list[int]) -> float:
    """Compute bitwise accuracy between two bit sequences."""
...
def generate_random_messages(
    ...
    """Generate random bit sequences for testing."""
    if seed is not None:
        random.seed(seed)
    return [
