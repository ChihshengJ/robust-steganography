...
def generate_covertext(length: int) -> list[str]:
    ...
    _, tokens, _ = embedder.embed(
        keys=keys, h=history, m=message, delta=delta, c=5, covertext_length=length
    )
...
def plot_repeated_ngrams(
    ...
    plt.errorbar(
        n_gram_values, distinct_values, yerr=confidence_intervals, fmt="o-", capsize=5
    )
...
def plot_watermark_length_delta(
    delta_values,
    n_bits,
    epsilon=0.05,
    safety_factor=1,
    ...
    for delta in delta_values:
        required_length = calculator.get_covertext_length(
            n=n_bits, epsilon=epsilon, delta=delta, safety_factor=safety_factor
        )
    ...
    plt.title(
        f"Covertext Length vs. δ (n = {n_bits}, ε = {epsilon}, Safety Factor = {safety_factor})"
    )
...
def plot_watermark_length_m(
    ...
    for m in m_lengths:
        required_length = calculator.get_covertext_length(
            n=m, epsilon=epsilon, delta=delta, safety_factor=safety_factor
        )
        covertext_lengths.append(required_length)
    ...
    plt.ylabel("Required Covertext Length (tokens)")
    plt.title(
        f"Covertext Length vs. Message Length (δ = {delta}, ε = {epsilon}, Safety Factor = {safety_factor})"
    )
    plt.grid(True)
...
# -----------------------------------------------------------------------------
def plot_watermarking_tampering_effect(
    tampering_types,
    tampering_percentages,
    baseline_recovery,
        ...
        for tp in tampering_percentages:
            ...
            for mode in mode_list:
                ...
                recovered_message = extractor.extract(
                    keys=keys, ct=attacked_text, h=history, c=c
                )
                ...
                for _ in range(runs):
                    ...
                    ):  # Perfect recovery if the lists match exactly.
                        perfect_count += 1
...
def main():
    ...
    plot_watermarking_tampering_effect(
        tampering_types, tampering_percentages, recovery_accuracy
    )
