...
from watermarks.utils import detect
...
def measure_recovery_rates(
    ...
    for msg_size in message_sizes:
        ...
        required_length = calculator.get_covertext_length(
            n=msg_size, epsilon=epsilon, delta=delta, safety_factor=safety_factor
        )
        ...
        for _ in range(num_trials):
            ...
            recovered_counters, _ = extractor.extract(
                keys=keys, h=history, ct=watermarked_text, c=5
            )
...
def main():
    ...
    plot_recovery_results(message_sizes, success_rates, confidence_intervals)
