...
class TestErrorCorrection(unittest.TestCase):
    def setUp(self):
        ...
        self.conv_codes = {
            size: ConvolutionalCode(block_size=size) for size in self.block_sizes
        }
    ...
    def test_error_correction(self):
        ...
        self.assertEqual(message, decoded, "RepetitionCode failed to correct errors")
        ...
        self.assertEqual(message, decoded, "ConvolutionalCode failed to correct errors")
    ...
    def _introduce_errors(self, bits, error_rate):
        ...
        corrupted = bits.copy()
        num_errors = int(
            len(corrupted) * error_rate
        )  # Calculate expected number of errors
        indices = random.sample(
            range(len(corrupted)), num_errors
        ...
        for message in edge_cases:
            for block_size in self.block_sizes:
                ...
                try:
                    ...
                    self.fail(
                        f"RepetitionCode failed with edge case {message}, block_size {block_size}: {str(e)}"
                    )
                ...
                try:
                    ...
                    self.fail(
                        f"ConvolutionalCode failed with edge case {message}, block_size {block_size}: {str(e)}"
                    )
