...
class RepetitionCode(ErrorCorrection):
    ...
    def _pad_input(self, bits):
        ...
        num_padding = self._calculate_padding(len(bits), padding_field_size, max_padding_value)
    ...
    def _calculate_padding(self, bits_length, padding_field_size, max_padding_value):
        ...
        try:
            ...
            raise ValueError("No modular inverse exists; check that repetitions and block_size are coprime.")
    ...
    def _unpad_output(self, bits):
        ...
        return bits[padding_field_size:-num_padding] if num_padding > 0 else bits[padding_field_size:]
...
class ConvolutionalCode(ErrorCorrection):
    ...
    def encode(self, input_bits):
        ...
        # Pad the input bits
        padded_input_bits, num_padding_bits, num_padding_bits_field_size = self._pad_input(input_bits, n, self.K)
    ...
    def _pad_input(self, input_bits, n, K):        
        ...
        num_padding_bits_bits = [int(b) for b in format(num_padding_bits, f'0{num_padding_bits_field_size}b')]
        
        # Add padding bits after the field size but before the input
        padded_input_bits = num_padding_bits_bits + [0] * num_padding_bits + padded_input_bits
    ...
    def decode(self, received_bits):
        ...
        for time in range(num_time_steps):
            received_symbols = received_bits[num_output_bits * time:num_output_bits * (time + 1)]
            current_metrics = {}
            ...
            for prev_state in path_metrics[time]:
                ...
                for input_bit in [0, 1]:
                    ...
                    prev_state_bits = [int(b) for b in format(prev_state, f'0{self.K-1}b')]
                    ...
                    branch_metric = sum([abs(received_symbols[i] - expected_symbols[i]) 
                                      for i in range(num_output_bits)])
                    total_metric = prev_metric + branch_metric
                    
                    # Update path metrics if better path found
                    if (next_state not in current_metrics or 
                        total_metric < current_metrics[next_state]['metric']):
        ...
        # Find best path
        final_state = min(path_metrics[-1], 
                         key=lambda s: path_metrics[-1][s]['metric'])
