...
class LanguageModel(Protocol):
    """Minimal structural type for the only model capability this attack uses:
    a ``tokenizer`` that round-trips text <-> token ids. Any watermarks
    ``LanguageModel`` (e.g. ``GPT2Model``) satisfies this, so the attacks module
    needs no import dependency on ``watermarks``.
    """
