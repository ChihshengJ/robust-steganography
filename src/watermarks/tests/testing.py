...
def find_delta(
    ...
    deltas = [0.00001, 0.0001, 0.001, 0.01, 0.1, 0.5]
...
def batch_test(
    ...
    print("\nFinal Results:")
    print(
        f"Message Matches: {message_matches}/{num_tests} ({message_matches / num_tests * 100:.1f}%)"
    )
    print(
        f"Token Matches: {token_matches}/{num_tests} ({token_matches / num_tests * 100:.1f}%)"
    )
