...
from watermarks import SmoothCovertextCalculator
...
def test_smooth_calculator_scaling():
    ...
    # Base case
    base_length = calculator.get_covertext_length(n=1, epsilon=0.05, delta=0.1)

    # Double message length should roughly double required length
    double_msg_length = calculator.get_covertext_length(n=2, epsilon=0.05, delta=0.1)
