...
def simple_prompt_builder(self, history: list[str], system_prompt: str, covered: set[str]) -> tuple[str, str]:
    return system_prompt, " ".join(history)

def check_exact_duplicate(self, response: str, existing: set[str]) -> bool:
...
def update_history_as_list(initial_history: list[str], steps: list["StepChoice"]) -> list[str]:
...
class RejectionSampler:
    def __init__(
        ...
        self.build_prompt: Callable[[Any, str, set[str]], tuple[str, str]] = prompt_builder or simple_prompt_builder
        self.check_duplicate: Callable[[str, set[str]], bool] = duplicate_checker or check_exact_duplicate
        self.clean_response: Callable[[str, Any], str] = response_cleaner
    ...
    def sample(
        ...
        while attempts < max_attempts:
            system_prompt_mod, prompt = self.build_prompt(history, system_prompt, covered)
            response = generate_response(prompt, system_prompt_mod, max_length, temperature)
            ...
            if np.array_equal(sampled_bits, desired_bits):
                ...
                if not collect_alternatives or len(matches) >= max_alternatives:
                    return SampleResult(success=True, matches=matches, attempts_used=attempts)

        return SampleResult(success=len(matches) > 0, matches=matches, attempts_used=attempts)
...
class BacktrackingEncoder:
    def __init__(
        ...
        self.update_history: Callable[[Any, list[StepChoice]], Any] = history_updater or update_history_as_list
    ...
    def encode(
        ...
        while i < len(chunks):
            ...
            collect_alternatives = (i != len(chunks) - 1) and self.config.collect_alternatives
            max_attempts = int(self.config.max_attempts_per_step * 1.5) if i == 0 else self.config.max_attempts_per_step
            ...
            if result.success:
                ...
                if backtracks_used > self.config.max_backtracks:
                    raise EncodingError(f"Exceeded maximum backtracks ({self.config.max_backtracks})")

                target = self._find_backtrack_target(steps)
                if target < 0:
                    raise EncodingError(f"No backtrack target available at position {i}")
        ...
        return [choice.message for choice in steps], [choice.embedding for choice in steps]
    ...
    def _log_success(self, position: int, total: int, result: SampleResult) -> None:
        alt_info = f" (+{len(result.matches) - 1} alternatives)" if len(result.matches) > 1 else ""
