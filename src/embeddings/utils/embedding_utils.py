...
def compute_embeddings_concurrently(texts, engine, client):
    ...
    with ThreadPoolExecutor() as executor:
        ...
        future_embeddings = executor.map(
            lambda text: get_embedding(text, engine, client), texts
        )
...
def compute_embeddings_pair_concurrently(texts1, texts2, engine, client):
    ...
    with ThreadPoolExecutor() as executor:
        ...
        future_embeddings_1 = executor.map(
            lambda text: get_embedding(text, engine, client), texts1
        )
        future_embeddings_2 = executor.map(
            lambda text: get_embedding(text, engine, client), texts2
        )
...
def compute_embeddings_pair(texts1, texts2, normalize, engine, client):
    if texts1 == texts2:
        embeddings_1, embeddings_2 = compute_embeddings_pair_concurrently_same(
            texts1, engine, client
        )
    else:
        embeddings_1, embeddings_2 = compute_embeddings_pair_concurrently(
            texts1, texts2, engine, client
        )
