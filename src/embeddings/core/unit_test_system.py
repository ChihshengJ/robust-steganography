...
class SummarySystem:
    ...
    def hide_message(self, data: Any, problem: str) -> str:
        """
        Given an problem, generate necessary behaviors (k+length) listed in descending order of importance, subdivided into High (K), Medium, and Low priority.
        Ask the LLM to generate all unit tests for high priority behaviors (hopefully deterministic).
        Then ask the LLM to generate the next test and hash it efficiently, avoid duplicate facts by maintaining a list of known facts.
    ...
    def _encode(self, chunks: list[list[int]], history: str) -> list[str]:
        ...
        for chunk in chunks:
            while not np.array_equal(sampled_bits, chunk):
                ...
                print(f"optional test: {processed_response}")
                embedding = (
                ...
                if np.array_equal(sampled_bits, chunk):
                    ...
                    print(f"updated prohibited_tests: {prohibited}")
        return facts
