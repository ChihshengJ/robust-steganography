...
def clean_response(response: str, history: dict) -> str:
    ...
    response = re.sub(
        r"^(?:Step\s+\d+\s*[:.\-]\s*)", "", response, flags=re.IGNORECASE
    ).strip()
...
class CotSystem(StegSystem):
    ...
    def recover_message(self, stego_text: str, **kwargs) -> Any:
        ...
        print(f"{'=' * 60}")
        print(
            f"  Expected: {expected_total} steps (base={self.key}, encoding={expected_optional})"
        )
        ...
        if len(optional_claims) < expected_optional:
            print(
                f"  WARNING: Only {len(optional_claims)} encoding claims, expected {expected_optional}"
            )
    ...
    def _generate_base_steps(self, question: str) -> list[str]:
        ...
        for line in response.strip().split("\n"):
            ...
            line = re.sub(
                r"^(?:Step\s+\d+\s*[:.\-]\s*)", "", line, flags=re.IGNORECASE
            ).strip()
            if line:
                steps.append(line)
    ...
    def _decompose(self, text: str, expected_total: int) -> list[str]:
        ...
        if len(claims) != expected_total:
            print(
                f"  Decomposition returned {len(claims)} claims, expected {expected_total}"
            )
